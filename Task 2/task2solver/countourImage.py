...
def	DetectSubContour(img, contour_dict, org_cont, cmarkeralpha):
	...
	for key in key_list:
		...
		if key.isnumeric():
			...
			temp[0:2,:], temp[:,0:2] =  255,255
			temp[ temp.shape[0]-2:temp.shape[0]] , temp[:,temp.shape[1]-2:temp.shape[1]]  = 255,255
			...
			if  len(cont) > 1: 
				...
				temp1  =  tempvar[x1+2:x1+h1-2,y1+2:y1+w1-2]



				if stflag:
					contourSaver(key,temp1)  
				...
				cv2.putText(temp, str(key) ,(x,y), font, 1, (255,255,255),2,cv2.LINE_8)
	...
	for key in test_keys:
		...
		temp[0:3,:], temp[:,0:3] =  255,255
		temp[ temp.shape[0]-3:temp.shape[0]] , temp[:,temp.shape[1]-3:temp.shape[1]]  = 255,255


		temp_gray = GrayConverter(temp)
		ret,thresh = ThresholdSubtractor(temp_gray, 127, 255, 0)
...
def ContourDrawer(image, contour_dict):
	...
	for i,j in zip(range(len(empty)), contour_dict.keys()):
		...
		cv2.putText(image, str(j) ,(x,y), font, 1,(0,0,255),2,cv2.LINE_8)
		cv2.drawContours(image,empty,i,(50,50,255),2)
